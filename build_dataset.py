# -*- coding = utf-8 -*-
# 开发人员：写代码的医生Yin-2020
# 开发时间：2021-05-25下午 10:30
# 文件名称：build_dataset.py
# 开发工具：PyCharm
...
class Mydata(Dataset):
    def __init__(self, root, resize, mode):
        ...
        for name in sorted(os.listdir(root)):
            if not os.path.isdir(os.path.join(root, name)):
                ...
                self.classifylabel[name] = len(self.classifylabel.keys())  #得到分类名标签对应的数字标签
    ...
    def load_csv(self, filename):
        if not os.path.exists(os.path.join(self.root, filename)):  #如果没有，将生成文件的路径和标签保存在一个filename.csv文件里
            images_path_list = []
            for name in self.classifylabel.keys():    #循环遍历完成后得到一个包含所有图片路径的列表
            ...
            random.shuffle(images_path_list)   #将这个列表随机化
