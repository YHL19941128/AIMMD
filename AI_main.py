...
def train(epoch):
    ...
    print('第[%d]个epoch的loss: %.3f' % (epoch + 1, train_epoch_loss))
...
if __name__ == '__main__':
    ...
    if visdom_switch == 'on':
        ...
        viz.line([0], [-1], win='val_loss', opts=dict(title='val_loss'))  # 定义val_loss曲线的初始点
        viz.line([0], [-1], win='train_acc', opts=dict(title='train_acc'))  # 定义train_acc曲线的初始点。
        viz.line([0], [-1], win='val_acc', opts=dict(title='val_acc'))  # 定义val_acc曲线的初始点。
    ...
    for epoch in range(epochs):
        ...
        train_batch_loss, train_epoch_loss, train_accuracy= train(epoch)  # 经过一轮训练返回了batchloss（列表，连接需要用extend）, epoch_loss, accuracy
        ...
        if visdom_switch == 'on':
            ...
            viz.line([train_accuracy], [epoch], win='train_acc',update='append')  # 定义train_acc曲线的初始点。
        ...
        if epoch % 1 == 0:  # 设置经过几个epoch的训练验证一次
            ...
            if val_accuracy > best_accuracy:
                best_epoch = epoch  # 记录最好的训练轮次
                best_accuracy = val_accuracy  # 记录最好的验证准确度
                torch.save(model.state_dict(), savepath + r'\bestmodel.mdl')  # 保存当前最好的模型状态
    ...
    torch.save(model.state_dict(), savepath + r'\lastmodel.mdl')  # 保存最后一次的模型状态

    print('***********val_best acc:%d %%*************'%(best_accuracy) +'\n'+ '***********val_best epoch:%d*************'%(best_epoch+1))  # 训练完打印最佳模型的提示信息

    model.load_state_dict(torch.load(savepath + r'\bestmodel.mdl'))  # 将保存的最优模型加载进来
    print('------Ok!--------')
